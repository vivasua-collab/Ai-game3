...
**Решение:** Ручная чистка в Unity Editor: Open Scene → найти Missing Scripts → удалить.

### 1.2 Синий фон между спрайтами тайлов
**Файл:** `Assets/Scripts/Tile/TileMapController.cs`, `TestLocationGameController.cs:293`
**Суть:** Sprite.Create использует pixelsPerUnit=64, но размер ячейки Grid=2×2 юнита. Нужно pixelsPerUnit=32 (64px / 2 единицы).
**Решение:** Изменить pixelsPerUnit с 64 на 32 в Sprite.Create() вызовах.

### 1.3 EventController — активные события теряются при загрузке
**Файл:** `Assets/Scripts/World/EventController.cs`
**Суть:** LoadSaveData() не восстанавливает activeEvents. Активные события исчезают.
**Решение:** Реализовать восстановление ActiveEvent из ID.

### 1.4 ResourcePickup — тихая потеря предметов
**Файл:** `Assets/Scripts/Tile/ResourcePickup.cs`
**Суть:** Когда itemData==null и InventoryController найден, возвращает true, но предмет не добавляется.
**Решение:** Возвращать false, если предмет не добавлен.
